...
class MonitoringActivity : AppCompatActivity() {
    ...
    private fun loadData() {
        ...
        lifecycleScope.launch {
            launch { db.gangguanDao().countAll().collectLatest       { runOnUiThread { b.tvTotal.text    = it.toString() } } }
            launch { db.gangguanDao().getByStatus("terverifikasi").collectLatest { runOnUiThread { b.tvDiproses.text = it.size.toString() } } }
            launch { db.gangguanDao().getByStatus("selesai").collectLatest  { runOnUiThread { b.tvSelesai.text  = it.size.toString() } } }
            launch {
